...
class SteamAuth implements SteamAuthInterface
{
    ...
    public function __construct(Request $request)
    {
        ...
        $redirect_url = Config::get('steamauth.redirect_url');
        $this->authUrl = $this->buildUrl(
            url($redirect_url, [], Config::get('steamauth.https'))
        );
    ...
    {
        if (is_null($return)) {
            $return = url('/', [], Config::get('steamauth.https'));
        }
        ...
        $realm = Config::get('steamauth.realm', $this->request->server('HTTP_HOST'));
        $params = [
            'openid.ns' => self::OPENID_NS,
            'openid.mode' => 'checkid_setup',
            'openid.return_to' => $return,
            'openid.realm' => (Config::get('steamauth.https') ? 'https' : 'http').'://'.$realm,
    ...
    public function parseInfo(): void
    {
        ...
        if (empty(Config::get('steamauth.api_key'))) {
            throw new RuntimeException('The Steam API key has not been specified.');
        }

        $response = $this->guzzleClient->request(
            'GET',
            sprintf(self::STEAM_INFO_URL, Config::get('steamauth.api_key'), $this->steamId)
        );
